        ...
        <div class="modal-overlay" v-if="activeModal === 'register'" @click="closeModal">
            <div class="modal-content" @click.stop>
                ...
                <form @submit.prevent="submitRegister">
                    ...
                    <div class="form-group" v-if="employees.length">
                        <label for="regEmployee">{{ t('sales_employee') || 'موظف المبيعات (اختياري)' }}</label>
                        <select v-model="registerForm.employee_id">
                            <option value="">{{ t('select_employee') || '-- اختر موظف المبيعات --' }}</option>
        ...
        <div class="modal-overlay" v-if="activeModal === 'subscribe'" @click="closeModal">
            <div class="modal-content" @click.stop>
                ...
                <form @submit.prevent="submitSubscribe">
                    ...
                    <div class="form-group" v-if="employees.length">
                        <label for="subEmployee">{{ t('sales_employee') || 'موظف المبيعات (اختياري)' }}</label>
                        <select v-model="subscribeForm.employee_id">
                            <option value="">{{ t('select_employee') || '-- اختر موظف المبيعات --' }}</option>
                            <option v-for="emp in employees" :key="emp.id" :value="emp.id">{{ emp.name }}</option>
                        </select>
                    </div>
...
const registerForm = reactive({ name: '', phone: '', email: '', password: '', employee_id: '' });
const subscribeForm = reactive({ name: '', phone: '', email: '', address: '', password: '', employee_id: '' });
const employees = ref([]);

const fetchSalesEmployees = async () => {
    try {
        const res = await axios.get('/api/v1/sales-employees');
        if (res.data?.success) {

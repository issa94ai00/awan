    ...
    <div class="secondary-navbar-page">
        <div class="page-header">
            <div class="page-title">
                <h1>{{ $t('secondary_navbar') }}</h1>
                <p>{{ $t('secondary_navbar_desc') }}</p>
            </div>
            <div>
        ...
        <el-card shadow="hover">
            <template #header>
                <div class="card-header">
                    <div>
                        <h2>{{ $t('navbar_items') }}</h2>
                        <p class="header-note">{{ $t('navbar_items_desc') }}</p>
                    </div>
                </div>
            </template>
                ...
                <div v-for="(element, index) in items" :key="element.id" class="nav-item-card" :class="{ inactive: !element.active }">
                    <div class="item-header">
                        ...
                        <el-tag size="small" :type="element.type === 'dropdown' ? 'primary' : 'info'">
                            {{ element.type === 'dropdown' ? ($t('dropdown')) : ($t('link')) }}
                        </el-tag>
                    ...
                    <div v-if="element.type === 'dropdown'" class="children-section">
                        <div class="children-header">
                            <span class="children-title">{{ $t('sub_items') }}</span>
        ...
        <el-dialog
            ...
            :title="dialog.isNew ? ($t('add_item')) : ($t('edit_item'))"
            width="600px"
        ...
        >
            <el-form :model="dialog.form" label-position="top">
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item :label="$t('arabic_label')">
                            <el-input v-model="dialog.form.label_ar" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item :label="$t('english_label')">
                ...
                <el-form-item v-if="dialog.form.type === 'link'" :label="$t('route')">
        ...
        <el-dialog
            ...
            :title="childDialog.isNew ? ($t('add_sub_item')) : ($t('edit_sub_item'))"
            width="500px"
            :close-on-click-modal="false"
        >
            ...
            <template #footer>
                <el-button @click="childDialog.visible = false">{{ $t('cancel') }}</el-button>
                <el-button type="primary" @click="confirmChildDialog">{{ $t('confirm') }}</el-button>
            </template>
        </el-dialog>
    </div>
</template>
    ...
    return [
        {
            ...
            label_ar: t('nav_products'), label_en: 'Products', icon: 'fas fa-th-list',
            children: [
                { id: 'all_products', active: true, label_ar: t('all_products'), label_en: 'All Products', icon: 'fas fa-th-large', route: '/products' },
                { id: 'electronics', active: false, label_ar: t('electronics'), label_en: 'Electronics', icon: 'fas fa-laptop', route: '/products?category=electronics' },
                { id: 'clothing', active: false, label_ar: t('clothing'), label_en: 'Clothing', icon: 'fas fa-tshirt', route: '/products?category=clothing' },
                { id: 'home_garden', active: false, label_ar: t('home_garden'), label_en: 'Home & Garden', icon: 'fas fa-home', route: '/products?category=home' },
            ]
        },
        {
            id: 'featured', type: 'dropdown', active: true,
            label_ar: t('featured_products'), label_en: 'Featured Products', icon: 'fas fa-star',
            children: [
                { id: 'view_all_featured', active: true, label_ar: t('view_all_featured'), label_en: 'View All Featured', icon: 'fas fa-fire', route: '/featured-products' },
                { id: 'new_arrivals', active: true, label_ar: t('new_arrivals'), label_en: 'New Arrivals', icon: 'fas fa-sparkles', route: '/featured-products?new' },
                { id: 'best_sellers', active: true, label_ar: t('best_sellers'), label_en: 'Best Sellers', icon: 'fas fa-trophy', route: '/featured-products?best' },
            ]
        },
        {
            id: 'offers', type: 'dropdown', active: true,
            label_ar: t('special_offers'), label_en: 'Special Offers', icon: 'fas fa-tag',
            children: [
                { id: 'current_offers', active: true, label_ar: t('current_offers'), label_en: 'Current Offers', icon: 'fas fa-fire', route: '/special-offers' },
                { id: 'flash_sales', active: true, label_ar: t('flash_sales'), label_en: 'Flash Sales', icon: 'fas fa-bolt', route: '/special-offers?flash' },
                { id: 'clearance', active: true, label_ar: t('clearance'), label_en: 'Clearance', icon: 'fas fa-percent', route: '/special-offers?clearance' },
            ]
        },
        {
            id: 'categories', type: 'link', active: true,
            label_ar: t('nav_categories'), label_en: 'Categories', icon: 'fas fa-folder', route: '/categories'
        },
        {
            id: 'contact', type: 'link', active: true,
            label_ar: t('contact_us_btn'), label_en: 'Contact Us', icon: 'fas fa-headset', route: '/contact'
        },
    ];
}
...
function confirmDialog() {
    if (!dialog.form.label_ar) {
        ElMessage.warning(t('please_enter_label'));
        return;
    }
    if (dialog.isNew) {

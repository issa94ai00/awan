    ...
    <div class="purchases-page purchases-suppliers">
        <div class="page-header">
            <div class="page-title">
                <h1>{{ $t('suppliers') }}</h1>
                <p>{{ $t('the_supplier_interface_is_designed') }}</p>
            </div>
            <el-input v-model="searchQuery" :placeholder="$t('search_by_name_email_or_phone')" clearable class="search-input" />
        </div>
        ...
        <el-card shadow="hover" class="table-panel">
            ...
            <div v-else>
                ...
                <div v-if="!filteredSuppliers.length" class="empty-state">{{ $t('there_are_no_suppliers_matching') }}</div>
            </div>
        </el-card>
    </div>
</template>

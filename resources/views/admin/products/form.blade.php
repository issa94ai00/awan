        ...
        <div class="tab-content" id="images">
            <div class="form-group">
                ...
                @if(isset($product) && $product->image_main)
                    <div class="image-preview mt-3" style="position: relative; display: inline-block;">
                        <img src="{{ $product->image_main_url }}" alt="{{ $product->name_ar }}" style="max-width: 200px; border-radius: 8px;">
                        <button type="button" class="remove-main-image btn btn-danger btn-sm" 
                        ...
                        <input type="hidden" name="remove_main_image" value="0" id="remove_main_image_input">
                    </div>
                @endif
                <div class="new-image-preview mt-3" style="display: none;"></div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="new-gallery-preview mt-3" style="display: flex; flex-wrap: wrap; gap: 10px;"></div>
                            ...
                            <div class="gallery-images" style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 10px;">
                                @foreach($galleryImages as $index => $image)
                                    <div class="gallery-item" style="position: relative; display: inline-block;">
                                        ...
                                        <input type="checkbox" name="remove_gallery_images[]" value="{{ $image }}" class="remove-image-checkbox" style="display: none;">
                                    </div>
                                @endforeach
                            </div>
                        </div>
...
// Main image removal toggle
let removeMainImageActive = false;
document.querySelector('.remove-main-image')?.addEventListener('click', function() {
    const previewContainer = this.closest('.image-preview');
    const removeInput = document.getElementById('remove_main_image_input');
    
    removeMainImageActive = !removeMainImageActive;
    
    if (removeMainImageActive) {
...
document.querySelectorAll('.remove-gallery-image').forEach(button => {
    button.addEventListener('click', function() {
        ...
        const checkbox = galleryItem.querySelector('.remove-image-checkbox');
...
// Main image preview with animation
document.getElementById('image_main')?.addEventListener('change', function() {
    ...
    // Reset remove flag when uploading a new image
    const removeInput = document.getElementById('remove_main_image_input');
    if (removeInput) {
        removeInput.value = '0';
        removeMainImageActive = false;
        const removeBtn = document.querySelector('.remove-main-image');
        if (removeBtn) {
            const previewContainer = removeBtn.closest('.image-preview');
            if (previewContainer) previewContainer.style.display = 'none';
        }
    }
});

// Gallery images preview - replace instead of accumulate
document.getElementById('image_gallery')?.addEventListener('change', function() {
    const files = this.files;
    const previewContainer = document.querySelector('.new-gallery-preview');
    ...
    Array.from(files).forEach(file => {
        ...
        reader.onload = function(e) {
            ...
            imgContainer.classList.add('gallery-preview-item');
            ...
            // Remove button for individual preview items
            const removeBtn = document.createElement('button');
            removeBtn.type = 'button';
            removeBtn.innerHTML = '<i class="fas fa-times"></i>';
            removeBtn.style.cssText = 'position:absolute;top:-5px;right:-5px;width:24px;height:24px;border-radius:50%;padding:0;display:flex;align-items:center;justify-content:center;background:var(--danger);color:white;border:none;cursor:pointer;';
...
// Helper to remove a specific file from the file input

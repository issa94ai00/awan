...
class FinancialAnalyticsService
{
    ...
    {
        return JournalEntryHeader::whereBetween('entry_date', [$fromDate, $toDate])
    ...
    {
        $entries = JournalEntryHeader::whereBetween('entry_date', [$fromDate, $toDate])
    ...
    {
        $cashInflows = JournalEntryHeader::whereBetween('entry_date', [$fromDate, $toDate])
            ->whereHas('lines', function ($query) {
        ...
        $cashOutflows = JournalEntryHeader::whereBetween('entry_date', [$fromDate, $toDate])
            ->whereHas('lines', function ($query) {
                $query->where('debit', '>', 0);
            })
            ->with('lines')
    ...
    public function getAccountsAging($asOfDate): array
    {
        ...
        foreach ($invoices as $invoice) {
            ...
            if ($daysOverdue <= 0) {
                ...
                $agingBuckets['1_30_days'] += $invoice->total;
            } elseif ($daysOverdue <= 60) {
                $agingBuckets['31_60_days'] += $invoice->total;
            } elseif ($daysOverdue <= 90) {
                $agingBuckets['61_90_days'] += $invoice->total;
            } else {
                $agingBuckets['over_90_days'] += $invoice->total;
            }
        }

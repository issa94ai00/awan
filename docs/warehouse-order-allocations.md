...
## 9. مسار واحد لدورة حياة الطلب

التخصيص والحجز والشحن والترحيل تملكها **`SalesOrderWorkflowService` وحدها**.

كان هناك مسار ثانٍ موازٍ (`EnhancedSalesOrderController@allocateInventory` عبر
`OrderAllocationService::allocateOrder`) يحجز بآلية مختلفة، ويتجاهل التوزيع المسجَّل على
بنود الطلبية، وينقل الطلب إلى `processing` **بلا فاتورة ولا قيد ولا تكلفة ولا قائمة تجهيز**
— فالطلب الذي يمر به لا يصل إلى الدفاتر إطلاقاً. وقد كان معطلاً أصلاً: كان يرشِّح على
`available_stock` وهو accessor وليس عموداً، فيرفع `Unknown column` عند أول استدعاء.

المسار الآن موحَّد:

| الحاجة | الجهة المسؤولة |
|--------|----------------|
| اقتراح التوزيع (خطة فقط، بلا حجز) | `OrderAllocationService::suggestAllocations` |
| تغطية المستودعات وترشيح الوجهة | `SalesOrderWorkflowService::routingOptions` / `pickWarehouse` |
| قراءة وتعديل خطة المصادر | `SalesOrderWorkflowService::sourcingPlan` / `saveSourcingPlan` |
| الحجز والفاتورة والقيد وقوائم التجهيز | `SalesOrderWorkflowService::transitionTo` |

> `POST /sales-orders/enhanced/{id}/allocate` و`/check-fulfillment` ما زالا يعملان
> بنفس عقدهما، لكنهما ينفذان عبر دورة الحياة أعلاه.

### 9.1 قاعدة محاسبية: المخزون يُرحَّل لحساب مستودعه

لكل مستودع حساب مخزون `1005-{id}` تحت الأصل `1005`. **الإدخال والإخراج يجب أن يلمسا
الحساب نفسه**:

- البيع يُدين التكلفة ويُقرن **حساب كل مستودع** بحصته (`postCostOfGoodsSoldBySource`).
- الشراء يُدين **حساب المستودع المستلم** (`postGoodsReceipt`).

كان الشراء يُدين الحساب المجمَّع بينما البيع يُقرن حسابات المستودعات، فكان رصيد كل
مستودع ينزل ولا يرتفع أبداً — يهبط للسالب مهما امتلأ الرف، ويتضخّم الحساب الأب. لا
يمكن عندها قراءة قيمة مخزون أي مستودع من الدفاتر.

الحساب يُنشأ عند أول ترحيل يحتاجه (`inventoryAccountIdFor`)، فالمستودع المُضاف بعد
تشغيل الـ migration لا يسقط صامتاً إلى الحساب المجمَّع.

> **ملاحظة للتطبيق:** الإصلاح **للأمام فقط** — القيود المُرحَّلة سابقاً إلى الحساب
> المجمَّع تبقى كما هي، فالتاريخ المحاسبي لا يُعاد كتابته. أرصدة المستودعات تبدأ من
> تاريخ الإصلاح؛ وإن أردتم مطابقتها بأثر رجعي فذلك قيد تسوية افتتاحي منفصل.

## 10. نقاط مفتوحة (توصيات مستقبلية)

- دعم تخصيصات المنتجات المركّبة (تفكيكها لمكونات قبل الاقتراح).
- حقول `batch_number` و`expiry_date` على التخصيص لدعم FEFO — تكلفة FIFO محسوبة
  حالياً على مستوى طبقات `inventory_cost_layers` لكل مستودع، لا على مستوى التخصيص.
- توحيد تعريف "المتاح": `suggestAllocations` يقرأ
  `quantity - reserved - damaged - quarantined`، بينما الحجز الفعلي يقرأ
  `available_quantity - reserved_quantity`. القيمتان تتطابقان طالما بقي
  `quantity = available + damaged + quarantined`، وهو ما تحفظه `InventoryService`
  لكن قد تخالفه بيانات مستوردة قديمة.
